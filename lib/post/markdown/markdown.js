...
module.exports = {
	...
	processCodeChunk: (text, highlightOptions) => {
		...
		if (!lang) {
			...
			try {
				const { value } = highlight(trimFix, { language: lang });
				return `<span class='code hljs'>${value}</span>`;
			} catch { /* ignore */ } //if manually-set language fails, fall back to plain
		}
		//else, auto highlight relevance threshold was too low, highlighting as lang failed, or lang was 'plain'
